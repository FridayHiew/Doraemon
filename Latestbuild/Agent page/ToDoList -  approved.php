...
<div class="topnav-2">
  ...
  <a class="active" href="ToDoList -  approved.php">Approved/Rejected</a>
  <a href="ToDoList -  return.php">Return</a>
</div>
...
<div class="div-head1"><b>To-Do List - Approved/Rejected</b></div>
<hr class="div-head1">
...
$sql = "SELECT * FROM dora_temp_user where RequestStatus='approved' OR RequestStatus='rejected'";
?>
...
<?php
    ...
    if(!empty($_REQUEST['selection1'])) {
		    ...
		    if(!empty($_REQUEST['selection2'])) {
				...
				$sql = "SELECT * FROM dora_temp_user where $selected LIKE '%" . $firstname ."%' AND RequestStatus='approved' OR $selected LIKE '%" . $firstname ."%' AND RequestStatus='rejected'";
			} else {
...
<form action="ToDoList -  approved.php" method="post">

<select name="selection1">
    <option value="">Please Select</option>
    <option value="MakerID">Originator</option>
	<option value="RequestModule">Description</option>
</select>
...
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
            ...
            echo '<tr>';
            echo '<th style="width:5%;">S/N</th>';
            echo '<th style="width:20%;">Last Action By</th>';
            echo '<th style="width:15%;">Description</th>';
			echo '<th style="width:15%;">Parameter</th>';
			echo '<th style="width:10%;">Status</th>';
			echo '<th style="width:15%;">Originator</th>';
			echo '<th style="width:25%;">Transaction Date/Time</th>';
            echo '</tr>';			
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            echo '<td> <a href="UserApproved.php?id='      . $row['Username'] .   '">'       . $row['RequestModule'] .           '</a></td>';		
