...
<div class="topnav-2">
  ...
  <a class="active" href="ToDoList - pending approval.php">Pending Approval</a>
...
<div class="div-head1"><b>To-Do List - Pending Approval</b></div>
<hr class="div-head1">
...
$sql = "SELECT * FROM dora_temp_user where RequestStatus='Pending Approval'";
?>
...
<?php
    ...
    if(!empty($_REQUEST['selection1'])) {
		    ...
		    if(!empty($_REQUEST['selection2'])) {
				...
				$sql = "SELECT * FROM dora_temp_user where $selected LIKE '%" . $firstname ."%' AND RequestStatus='Pending Approval'";
			} else {
...
<form action="ToDoList - pending approval.php" method="post">
...
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            echo '<td> <a href="UserPending.php?id='      . $row['Username'] .   '">'       . $row['RequestModule'] .           '</a></td>';		
