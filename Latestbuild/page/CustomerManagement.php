...
<!--------- Table Content ------------->
		<div class="div-head1"><b>Customer Management</b></div>
		<hr class="div-head1">
...
<?php
    ...
    if(!empty($_REQUEST['selection1'])) {
		    ...
		    if(!empty($_REQUEST['selection2'])) {
				...
				$sql = "SELECT * FROM dora_cust where $selected LIKE '%" . $firstname ."%' AND RequestStatus='draft'";
			} else {
...
$sql = "SELECT * FROM dora_cust where status='Active'";
$sn=1;
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_array($result)){
            echo '<tr>';
                ...
                echo '<td>' . $sn . '</td>';
                echo '<td> <a href="Update User.php?id='      . $row['LastName'] .   '">'       . $row['FirstName'] .           '</a></td>';
