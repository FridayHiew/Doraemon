...
<div style="padding:5px;"></div>
	...
	<div>
			<ul id="navlist">
			  ...
			  <li id="logout"><a href="../php/logout.php"><img src="../img/logout.gif" alt="logout" width="80px" height="80px" border="5px solid black";></a></li>
			</ul>
	</div>
...
$sql = "SELECT * FROM dora_temp_cust where RequestStatus='draft'";
?>
...
<?php
    ...
    if(!empty($_REQUEST['selection1'])) {
		    ...
		    if(!empty($_REQUEST['selection2'])) {
				...
				$sql = "SELECT * FROM dora_temp_cust where $selected LIKE '%" . $firstname ."%' AND RequestStatus='draft'";
			} else {
...
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
            ...
            echo '<th style="width:20%;">Maker ID</th>';
        ...
        while($row = mysqli_fetch_array($result)){
            echo '<tr>';	
            echo '<td>' . $sn . '</td>';							
            echo '<td> <a href="UserDraft.php?id='      . $row['MakerID'] .   '">'     . $row['MakerID']  .'</a></td>';		
            echo '<td>' . $row['LastUpdateBy'] . '</td>';	
			echo '<td>' . $row['RequestModule']  . '</td>'; 
